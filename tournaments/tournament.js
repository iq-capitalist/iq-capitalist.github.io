...
function displayTopPlayers(data) {
    if (!data.players || !Array.isArray(data.players) || data.players.length === 0) {
        console.warn('Нет данных об игроках для определения рекордов');
    ...
    const playersWithAccuracy = players.filter(player => {
        if (!player.correct_answers || !player.wrong_answers) return false;
        
        const totalAnswers = (player.correct_answers.fast || 0) + 
                           (player.correct_answers.medium || 0) + 
                           (player.correct_answers.slow || 0) + 
                           (player.wrong_answers.fast || 0) + 
                           (player.wrong_answers.medium || 0) + 
                           (player.wrong_answers.slow || 0);
        const correctAnswers = (player.correct_answers.fast || 0) + 
                             (player.correct_answers.medium || 0) + 
                             (player.correct_answers.slow || 0);
        
        // Возвращаем игрока с дополнительными полями
        player.accuracy = totalAnswers > 0 ? correctAnswers / totalAnswers : 0;
        player.correctAnswers = correctAnswers;
        player.totalAnswers = totalAnswers;
        
        return totalAnswers >= 10; // Минимум 10 ответов
    });
    
    if (playersWithAccuracy.length === 0) {
        document.getElementById('most-accurate-player').innerHTML = 
            '<span class="player-name">Нет данных</span>';
    } else {
        const mostAccuratePlayer = [...playersWithAccuracy].sort((a, b) => b.accuracy - a.accuracy)[0];
        document.getElementById('most-accurate-player').innerHTML = 
            `<span class="player-name">${mostAccuratePlayer.username || 'Неизвестный игрок'}</span>
    ...
    const playersWithFastAnswers = players.filter(player => {
        if (!player.correct_answers || !player.wrong_answers || !player.answers) return false;
        
        const fastAnswers = (player.correct_answers.fast || 0) + (player.wrong_answers.fast || 0);
        const totalAnswers = player.answers;
        
        // Возвращаем игрока с дополнительными полями
        player.fastAnswers = fastAnswers;
        player.fastPercent = totalAnswers > 0 ? (fastAnswers / totalAnswers) * 100 : 0;
        
        return totalAnswers >= 10; // Минимум 10 ответов
    });
    
    if (playersWithFastAnswers.length === 0) {
        ...
        document.getElementById('fastest-player').innerHTML = 
            `<span class="player-name">${fastestPlayer.username || 'Неизвестный игрок'}</span>
